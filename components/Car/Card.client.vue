...
<template>
  ...
  >
    <img @click="emit('favor', car.id)" :src="favored ? heartFilled : heartOutline" class="absolute w-7 right-5 top-2 z-20" alt="" />
    <div @click="navigateTo(`/car/${car.name}-${car.id}`)" class="flex h-full">
      <NuxtImg
          :src="car.url"
          alt=""
